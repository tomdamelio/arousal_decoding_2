...
class run_experiments:
    ...
    def __init__(self, directory, number_subject, high_pass_filter, shift_EDA,
                 tune_components, target, model ):
        self.directory           = directory
        self.number_subject      = number_subject
        self.high_pass_filter    = high_pass_filter
        self.shift_EDA           = shift_EDA
        self.tune_components     = tune_components
        self.target              = target
        self.model               = model
        self.eda                 = eda
        self.raw                 = raw
 
    def preprocess (self, extension = 'bdf'):
        """
        ...
        fname = op.join(self.directory, 's'+ self.number_subject + extension)
        ...
        eda.filter(self.high_pass_filter, 5, fir_design='firwin') 
    ...
    def compute_cov_matrices (self):
        ...
        for fb in pp.fbands:
            rf = self.raw.copy().load_data().filter(fb[0], fb[1])
    ...
    def compute_model_output (self):
        ...
        """
        events_shift = mne.make_fixed_length_events(self.eda, id=3000, start=1.5,
                                                    duration=pp.duration, overlap=2.0)

        eda_epochs_shift = Epochs(self.eda, events_shift, event_id=3000, tmin=0, tmax=pp.duration,
    ...
    def run_low_rank(self, n_components, X, y, cv, estimators, scoring, groups):
    ...
    def pipeline_all_subjects  (self,
                                directory        =  'data',
                                number_subject   =  'all',
...
run_experiments.pipeline_all_subjects(number_subject='01')
